...
exports.seed = async function (knex, Promise) {
	...
	let result = await knex('department').select('id').where({ name: "Elementary Student" }).first();
	...
	if (result) {
		let department_id = result.id;
		
		await knex('grade_level').returning(['id', 'name']).insert([
			{ name: "Grade 1", department_id: department_id, sy_id: sy_id[0] },
			{ name: "Grade 2", department_id: department_id, sy_id: sy_id[0] },
			{ name: "Grade 3", department_id: department_id, sy_id: sy_id[0] },
			{ name: "Grade 4", department_id: department_id, sy_id: sy_id[0] },
			{ name: "Grade 5", department_id: department_id, sy_id: sy_id[0] },
			{ name: "Grade 6", department_id: department_id, sy_id: sy_id[0] },
			{ name: "Grade 7", department_id: department_id, sy_id: sy_id[0] }
		])
